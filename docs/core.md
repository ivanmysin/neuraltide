...
`n_units` — число независимых популяций одного типа внутри одного объекта. Например, `IzhikevichMeanField(n_units=4)` описывает 4 независимые популяции с потциально разными параметрами. Все n_units обрабатываются одной батчированной матричной операцией.
...
Базовый класс для генератора входных сигналов. Генераторы используются для создания внешних входных сигналов, которые передаются в сеть через `NetworkGraph.declare_input()` и `pack_inputs()`.
